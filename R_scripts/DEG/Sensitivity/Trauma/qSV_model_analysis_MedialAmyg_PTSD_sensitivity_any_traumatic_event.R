...
mod = model.matrix(~Group + AgeDeath + Sex + Region + mitoRate + rRNA_rate + totalAssignedGene + RIN + overallMapRate + ERCCsumLogErr + snpPC1 + snpPC2 + snpPC3 + snpPC8 + snpPC9 + snpPC10 + any_traumatic_event, data = colData(rse_gene))
...
save(qsvBonf, qSVs, mod, modQsva, file = 'rdas/PTSD_qsvs_sensitivity_any_traumatic_event.Rdata')
...
mod = model.matrix(~onlyPTSD + AgeDeath + Sex + Region + mitoRate + rRNA_rate + totalAssignedGene + RIN + overallMapRate + ERCCsumLogErr + snpPC1 + snpPC2 + snpPC3 + snpPC8 + snpPC9 + snpPC10 + any_traumatic_event, data = colData(rse_gene))
...
modQsva = cbind(mod, qSVs)
save(qsvBonf, qSVs, mod, modQsva, file = 'rdas/PTSD_onlyPTSD_qsvs_sensitivity_any_traumatic_event.Rdata')
...
save(geneStats_MedialAmyg, file = "rdas/geneStats_DE_qSVA_sensitivity_any_traumatic_event_MedialAmyg_threeGroup.rda")
#dir.create("csvs",showWarnings = FALSE)
write.csv(geneStats_MedialAmyg, file = gzfile("csvs/geneStats_DE_qSVA_any_traumatic_event_MedialAmyg_threeGroup.csv.gz"))
