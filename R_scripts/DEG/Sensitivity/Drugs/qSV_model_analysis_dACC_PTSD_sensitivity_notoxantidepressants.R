...
demo <- read.csv("csvs/PTSD_cohort_Feb2019.csv",header=TRUE)
#change encoding of antidepressants_ssris to be 0 or 1 instead of true or false
#Note, changes the 3 "NA" samples to 1
demo$antidepressants_ssris <- ifelse(demo$antidepressants_ssris == "TRUE",1,0)
demo$antidepressants_ssris[is.na(demo$antidepressants_ssris)] <- 1
colnames(demo)[3] = "BrNum"
rownames(demo) = demo$BrNum
colData(rse_gene) = cbind(colData(rse_gene) , demo[rse_gene$BrNum,53, drop=FALSE])
...
save(mod,qsvBonf,qSVs,modQsva, file='rdas/PTSD_qsvs_sensitivity_notoxantidepressants.Rdata')
...
#filter for those without antidepressants_ssris (tox)
keepIndex2 = which(rse_gene$antidepressants_ssris == "0")
rse_gene <- rse_gene[, keepIndex2]
...
#Get rid of antidepressants_ssris column
colIndex2 <-  !grepl("antidepressants_ssris", colnames(mod))
mod <- mod[keepIndex2, colIndex2]
modQsva <- modQsva[keepIndex2,!grepl("antidepressants_ssris", colnames(modQsva))]
colnames(modQsva)[1] = "Int"
...
modQsva = cbind(mod, qSVs)
save(qsvBonf, qSVs, mod, modQsva, file = 'rdas/PTSD_onlyPTSD_qsvs_sensitivity_notoxantidepressants.Rdata')
...
save(geneStats_dACC, file = "rdas/dACC/geneStats_DE_qSVA_sensitivity_notoxantidepressants_dACC_threeGroup.rda")
write.csv(geneStats_dACC, file = gzfile("csvs/dACC/geneStats_DE_qSVA_sensitivity_notoxantidepressants_dACC_threeGroup.csv.gz"))
