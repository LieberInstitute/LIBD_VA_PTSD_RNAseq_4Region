...
#antidepressants (tox) vs non
sigGeneAntidepTox = topTable(eBGene,coef=18,
	p.value = 1,number=nrow(rse_gene), sort="none")
colnames(sigGeneAntidepTox) = paste0(colnames(sigGeneAntidepTox), "_AntidepTox")
...
save(geneStats_dACC, file = "rdas/dACC/geneStats_DE_qSVA_sensitivity_antidepressants_dACC_threeGroup.rda")
write.csv(geneStats_dACC, file = gzfile("csvs/dACC/geneStats_DE_qSVA_sensitivity_antidepressants_dACC_threeGroup.csv.gz"))

###Stuff we want

#Merge qSVA into one big genestats dataframe
geneStats = cbind(sigGenePTSD, sigGeneMDD, sigGeneDx ,sigGeneonlyPTSD, PTSDvsMDDPost,sigGeneAntidepTox)
geneStats = cbind(geneStats, rowData(rse_gene))

## write out qSVA-based stats
geneStats_dACC_antideptox = geneStats
save(geneStats_dACC_antideptox, file = "rdas/dACC/geneStats_DE_qSVA_sensitivity_antidepressants_dACC_threeGroup_antideptox.rda")
write.csv(geneStats_dACC_antideptox, file = gzfile("csvs/dACC/geneStats_DE_qSVA_sensitivity_antidepressants_dACC_threeGroup_antideptox.csv.gz"))
