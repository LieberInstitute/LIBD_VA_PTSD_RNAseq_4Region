...
mod = model.matrix(~Group + AgeDeath + Sex + Region + mitoRate + rRNA_rate + totalAssignedGene + RIN + overallMapRate + ERCCsumLogErr + snpPC1 + snpPC2 + snpPC3 + snpPC8 + snpPC9 + snpPC10 + lifetime_antidepress, data = colData(rse_gene))
...
save(qsvBonf, qSVs, mod, modQsva, file = 'rdas/PTSD_qsvs_sensitivity_lifetimeantidepressants.Rdata')
...
mod = model.matrix(~onlyPTSD + AgeDeath + Sex + Region + mitoRate + rRNA_rate + totalAssignedGene + RIN + overallMapRate + ERCCsumLogErr + snpPC1 + snpPC2 + snpPC3 + snpPC8 + snpPC9 + snpPC10 + lifetime_antidepress, data = colData(rse_gene))
...
modQsva = cbind(mod, qSVs)
save(qsvBonf, qSVs, mod, modQsva, file = 'rdas/PTSD_onlyPTSD_qsvs_sensitivity_lifetimeantidepressants.Rdata')
...
save(geneStats_BasoAmyg, file = "rdas/geneStats_DE_qSVA_sensitivity_lifetimeantidepressants_BasoAmyg_threeGroup.rda")
write.csv(geneStats_BasoAmyg, file = gzfile("csvs/geneStats_DE_qSVA_sensitivity_lifetimeantidepressants_BasoAmyg_threeGroup.csv.gz"))
