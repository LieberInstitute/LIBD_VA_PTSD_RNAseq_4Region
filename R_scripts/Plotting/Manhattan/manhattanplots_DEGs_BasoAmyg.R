...
# check for duplicated gene identifiers
length(unique(MDD_short$ensemblID_MDD))
length(MDD_short$ensemblID_MDD)
...
#maybe dont need chrsizetab/chrom_sizes here at all
#although still need to bin bc keep all DEGs from each chrom together (numbering is 1 to whatever for each chrom)
#use axis(,at= ) to set axis ticks at the chr not the pos
#include X and Y chrom? i dont think so..
#def dont include M
