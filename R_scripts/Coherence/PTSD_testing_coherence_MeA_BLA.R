...
BLAkeepIndex = which(rse_gene$Region == "BasoAmyg")
BLA_rse_gene <- rse_gene[,BLAkeepIndex]
MeAkeepIndex = which(rse_gene$Region == "MedialAmyg")
MeA_rse_gene <- rse_gene[,MeAkeepIndex]

#Only keep samples where we have both the BLA and the MeA
test <- which(BLA_rse_gene$BrNum %in% MeA_rse_gene$BrNum)
both_BLA_rse_gene <- BLA_rse_gene[,test]
test2 <- which(MeA_rse_gene$BrNum %in% both_BLA_rse_gene$BrNum)
both_MeA_rse_gene <- MeA_rse_gene[,test2]

#Check that the brain numbers are the same
both_MeA_rse_gene$BrNum <- as.character(both_MeA_rse_gene$BrNum)
unique(both_MeA_rse_gene$BrNum)
unique(both_BLA_rse_gene$BrNum)
identical(sort(both_MeA_rse_gene$BrNum), sort(both_BLA_rse_gene$BrNum))

identical(unique(both_MeA_rse_gene$BrNum), unique(both_BLA_rse_gene$BrNum))

#2 MeA for BrNum5415
table(both_MeA_rse_gene$BrNum, both_MeA_rse_gene$Region)
table(both_BLA_rse_gene$BrNum, both_BLA_rse_gene$Region)
#drop BrNum5415
BLAkeepIndex2 = which(both_BLA_rse_gene$BrNum == "Br5415")
both_BLA_rse_gene <- both_BLA_rse_gene[,-BLAkeepIndex2]

MeAkeepIndex2 = which(both_MeA_rse_gene$BrNum == "Br5415")
both_MeA_rse_gene <- both_MeA_rse_gene[,-MeAkeepIndex2]

identical(sort(both_MeA_rse_gene$BrNum), sort(both_BLA_rse_gene$BrNum))

#Subset into the different diagnoses
summary(as.factor(both_BLA_rse_gene$Group))
summary(as.factor(both_MeA_rse_gene$Group))

ptsdkeepIndex <- which(both_BLA_rse_gene$Group == "PTSD")
ptsd_both_BLA_rse_gene <- both_BLA_rse_gene[,ptsdkeepIndex]
MDDkeepIndex <- which(both_BLA_rse_gene$Group == "MDD")
mdd_both_BLA_rse_gene <- both_BLA_rse_gene[,MDDkeepIndex]
contkeepIndex <- which(both_BLA_rse_gene$Group == "Control")
cont_both_BLA_rse_gene <- both_BLA_rse_gene[,contkeepIndex]

ptsdkeepIndex <- which(both_MeA_rse_gene$Group == "PTSD")
ptsd_both_MeA_rse_gene <- both_MeA_rse_gene[,ptsdkeepIndex]
MDDkeepIndex <- which(both_MeA_rse_gene$Group == "MDD")
mdd_both_MeA_rse_gene <- both_MeA_rse_gene[,MDDkeepIndex]
contkeepIndex <- which(both_MeA_rse_gene$Group == "Control")
cont_both_MeA_rse_gene <- both_MeA_rse_gene[,contkeepIndex]

#Check that the number of samples in each diagnosis match
dim(cont_both_MeA_rse_gene)
dim(cont_both_BLA_rse_gene)
dim(ptsd_both_BLA_rse_gene)
dim(ptsd_both_MeA_rse_gene)
dim(mdd_both_MeA_rse_gene)
dim(mdd_both_BLA_rse_gene)

#Get the average expression for each gene across samples in each diagnosis
means_ptsd_both_BLA_rse_gene <- rowMeans(assay(ptsd_both_BLA_rse_gene))
means_mdd_both_BLA_rse_gene <- rowMeans(assay(mdd_both_BLA_rse_gene))
means_cont_both_BLA_rse_gene <- rowMeans(assay(cont_both_BLA_rse_gene))
means_cont_both_MeA_rse_gene <- rowMeans(assay(cont_both_MeA_rse_gene))
means_mdd_both_MeA_rse_gene <- rowMeans(assay(mdd_both_MeA_rse_gene))
means_ptsd_both_MeA_rse_gene <- rowMeans(assay(ptsd_both_MeA_rse_gene))

#Calculate Pearson correlations
cor(means_ptsd_both_MeA_rse_gene,means_ptsd_both_BLA_rse_gene)
cor(means_mdd_both_MeA_rse_gene,means_mdd_both_BLA_rse_gene)
cor(means_cont_both_MeA_rse_gene,means_cont_both_BLA_rse_gene)
...
#Note, the qSVs were generated across all four regions (dACC, DLPFC, BLA, and MeA) at once with "region" in the model (see `qSV_model_anaysis_BasoAmyg_PTSD_updated.R`). 
#The "keepIndex" is the only part specific to a region.

#load('rdas/PTSD_qsvs.Rdata')
#keepIndex = which(rse_gene$Region == "BasoAmyg")
#save(qsvBonf, qSVs, mod, modQsva, keepIndex, file = 'rdas/Coherence/PTSD_qsvs_BasoAmyg.Rdata')	
load('rdas/Coherence/PTSD_qsvs_BasoAmyg.Rdata')

#load('rdas/PTSD_qsvs.Rdata')
#keepIndex = which(rse_gene$Region == "MedialAmyg")
#save(qsvBonf, qSVs, mod, modQsva, keepIndex, file = 'rdas/Coherence/PTSD_qsvs_MedialAmyg.Rdata')
load('rdas/Coherence/PTSD_qsvs_MedialAmyg.Rdata')

regions <- c('BasoAmyg', 'MedialAmyg')
...
stopifnot(all(sapply(modQsva, nrow) == 319))
...
save(simple_rse, modQsva, file = 'rdas/Coherence/rse_and_modQsva_BasoAmyg_MedialAmyg.Rdata')
...
save(expr, file = 'rdas/Coherence/expr_BasoAmyg_MedialAmyg.Rdata')
...
save(cleaned_MDD, file = 'rdas/Coherence/cleaned_BasoAmyg_MedialAmyg_MDD.Rdata')
...
save(expr_ContMDD, file = 'rdas/Coherence/expr_BasoAmyg_MedialAmyg_MDD_subset.Rdata')
save(cleaned_ContMDD, file = 'rdas/Coherence/cleaned_BasoAmyg_MedialAmyg_MDD_subset.Rdata')
...
## Test paircor
test <- cor(cleaned_ContMDD[['BasoAmyg']][['geneRpkm']][1:100, ], cleaned_ContMDD[['MedialAmyg']][['geneRpkm']][1:100, ])
#test is subject vs subject matrix, 320x320
test2 <- paircor(cleaned_ContMDD[['BasoAmyg']][['geneRpkm']][1:100, ], cleaned_ContMDD[['MedialAmyg']][['geneRpkm']][1:100, ])
...
computecor <- function(exp) {
    ...
    res <- lapply(sets, function(feature) {
        ...
        paircor(exp[['BasoAmyg']][[feature]], exp[['MedialAmyg']][[feature]])
    })
    names(res) <- sets
    return(res)
}
...
save(indv_expr_ContMDD, indv_cleaned_ContMDD, file = 'rdas/Coherence/indv_corr_BasoAmyg_MedialAmyg_MDD_subset.Rdata')
...
setwd('/dcl02/lieber/ptsd/RNAseq/VA_PTSD_RNAseq/')
load('rdas/Coherence/rse_and_modQsva_BasoAmyg_MedialAmyg.Rdata',verbose=TRUE)
load('rdas/Coherence/expr_BasoAmyg_MedialAmyg.Rdata',verbose=TRUE)
...
save(cleaned_PTSD, file = 'rdas/Coherence/cleaned_BasoAmyg_MedialAmyg_PTSD.Rdata')
...
save(expr_ContPTSD, file = 'rdas/Coherence/expr_BasoAmyg_MedialAmyg_PTSD_subset.Rdata')
save(cleaned_ContPTSD, file = 'rdas/Coherence/cleaned_BasoAmyg_MedialAmyg_PTSD_subset.Rdata')
...
## Test paircor
test <- cor(cleaned_ContPTSD[['BasoAmyg']][['geneRpkm']][1:100, ], cleaned_ContPTSD[['MedialAmyg']][['geneRpkm']][1:100, ])
#test is subject vs subject matrix
test2 <- paircor(cleaned_ContPTSD[['BasoAmyg']][['geneRpkm']][1:100, ], cleaned_ContPTSD[['MedialAmyg']][['geneRpkm']][1:100, ])
...
save(indv_expr_ContPTSD, indv_cleaned_ContPTSD, file = 'rdas/Coherence/indv_corr_BasoAmyg_MedialAmyg_PTSD_subset.Rdata')
...
#plot MDD
pdf('pdf/Coherence/indv_box_BasoAmyg_MedialAmyg_MDD.pdf', useDingbats = FALSE)
...
#plot PTSD
pdf('pdf/Coherence/indv_box_BasoAmyg_MedialAmyg_PTSD.pdf', useDingbats = FALSE)
mapply(function(x, y, set, type) {
    m <- !is.na(x)
    dx <- colData(y)$Group[m]
...
#MDD
pdf('pdf/Coherence/indv_corr_vs_covariates_BasoAmyg_MedialAmyg_MDD.pdf', useDingbats = FALSE)
...
#PTSD
pdf('pdf/Coherence/indv_corr_vs_covariates_BasoAmyg_MedialAmyg_PTSD.pdf', useDingbats = FALSE)
lapply(c('AgeDeath'), function(var) {
    message(paste(Sys.time(), 'processing', var))
