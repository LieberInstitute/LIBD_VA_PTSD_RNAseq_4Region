...
#load network
load("rdas/WGCNA/constructed_network_signed_bicor_BasoAmyg_onlyPTSD.rda", verbose = TRUE)
#load rse objects
...
save(coefAdj, file="rdas/WGCNA/MEvsDx_BasoAmyg_onlyPTSD.rda")
...
#Warning in GOenrichmentAnalysis(net$colors, universe, organism = "human",  :
...
#save(GOenr, file="rdas/BasoAmyg/wgcna_BasoAmyg_GO_onlyPTSD.rda")
###########################################################
...
save(go_modules_basoamyg, file="rdas/BasoAmyg/wgcna_BasoAmyg_GO_clusterProfiler_onlyPTSD.rda")
...
#go_interest_padjust <- go_interest[go_interest$p.adjust <0.00001,]
#go_interest_padjust[,c("Cluster","Description")]

#go_interest_ordered <- go_interest[order(go_interest$p.adjust),]
#go_interest_ordered_topten <- go_interest_ordered[c(1:10),]
#go_interest_ordered_split <- split(go_interest_ordered$Description, go_interest_ordered$Cluster)
#go_interest_ordered_split_topten <- go_interest_ordered_split$'2'[1:10]
...
#pdf("pdf/testing.pdf")
#ggplot(data=go_interest_ordered_topten, aes(x = Description, y=-log10(p.adjust), fill=factor(Cluster))) +
#geom_bar(position="dodge",stat="identity", width = 0.5) + 
#coord_flip() +
#ylab("-log10(Adjusted P Value)") +
#ggtitle("GO BasoAmyg WCGNA Significant Modules")
#dev.off()
							
#go_interest <- go_df[go_df$Cluster == 5,]							
#go_interest_ordered <- go_interest[order(go_interest$p.adjust),]
#go_interest_ordered_topten <- go_interest_ordered[1:10,]
